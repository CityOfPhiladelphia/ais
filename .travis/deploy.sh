...
avoid_timeout & eb deploy $EB_ENV --timeout 20
...
if [ "$EB_BLUEGREEN_STATUS" = "Swap" ] ; then
  ...
  eb setenv --environment $EB_OLD_PROD_ENV SWAP=False --timeout 20 &
  eb setenv --environment $EB_NEW_PROD_ENV SWAP=False --timeout 20 &
  # Wait for both
  wait $(jobs -p)
