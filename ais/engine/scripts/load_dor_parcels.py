...
source = CONFIG['parcels']['dor']['source']
source_db = source['db']
source_table = source['table']
source_geom_field = source['geometry']['field']
source_geom_srid = source['geometry']['srid']
field_map = source['field_map']

street_table = 'street_segment'
parcel_table = CONFIG['parcels']['dor']['table']
parcel_error_table = CONFIG['parcels']['dor']['error_table']
parcel_error_polygon_table = CONFIG['parcels']['dor']['error_polygon_table']
WRITE_OUT = False
...
source_db = connect_to_db(CONFIG['db'][source_db])
...
if WRITE_OUT:
	...
	ais_db.drop_index(parcel_table, 'street_address')

	print('Deleting existing parcels...')
	ais_db.truncate(parcel_table)

	print('Deleting existing parcel errors...')
	ais_db.truncate(parcel_error_table)
	
	print('Deleting existing parcel error polygons...')
	ais_db.truncate(parcel_error_polygon_table)
...
street_stmt = '''
	...
	from {}
'''.format(street_table)
ais_db.c.execute(street_stmt)
street_rows = ais_db.c.fetchall()
...
source_parcels = source_db.read(source_table, source_fields, \
	geom_field=source_geom_field, where=source_where)
...
if WRITE_OUT:
	...
	ais_db.bulk_insert(parcel_table, parcels, geom_field='geometry', \
		chunk_size=50000)
	...
	ais_db.bulk_insert(parcel_error_table, errors, chunk_size=150000)
	del errors
	...
	ais_db.bulk_insert(parcel_error_polygon_table, error_polygons, \
		chunk_size=50000, geom_field='shape')
	del error_polygons

	print('Creating indexes...')
	ais_db.create_index(parcel_table, 'street_address')
