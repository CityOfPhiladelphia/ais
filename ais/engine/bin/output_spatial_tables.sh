...
the_el write temp_address_summary --connection-string $oracle_dsn_gis_ais --table-schema-path address_summary_schema.json --geometry-support sde --db-schema GIS_AIS --input-file address_summary_cleaned.csv --skip-headers
the_el swap_table temp_address_summary address_summary --connection-string $oracle_dsn_gis_ais #--select-users 'SDE, GIS_OPA, GIS_SDE_VIEWER' # Resolve grant select on address_sukmary to gis_opa with grant option' requirement either on gis_oddt.vw_property_summary end or incorporate into the-el swap_table oracle section
...
the_el write temp_dor_parcel_address_analysis --connection-string $oracle_dsn_gis_ais --table-schema-path dor_parcel_address_analysis_schema.json --geometry-support sde-char --db-schema GIS_DOR --input-file dor_parcel_address_analysis.csv --skip-headers
the_el swap_table temp_dor_parcel_address_analysis dor_parcel_address_analysis --connection-string $oracle_dsn_gis_ais
